...
void CreateFatTreeTopology(uint32_t k) {
    ...
    std::vector<NetDeviceContainer> DevicesContainer_vec; // holds all device pairs
    std::vector<Ipv4InterfaceContainer> InterfaceContainer_vec; // holds all device pairs
    ...
    for (uint32_t pod = 0; pod < numPods; ++pod) {
        for (uint32_t edgeIndex = pod * (k / 2); edgeIndex < (pod + 1) * (k / 2); ++edgeIndex) {
            for (uint32_t i = 0; i < k / 2; ++i) {
                ...
                Ipv4InterfaceContainer interface = ipv4.Assign(link);
                DevicesContainer_vec.push_back(link);
                InterfaceContainer_vec.push_back(interface);
                ipv4.NewNetwork();
            }
        }
    }
    ...
    ////////////////////////////////tcp application///////////////////////////////////////////
    uint16_t port = 50000; // Port for the application
    uint32_t maxBytes = 1024;


   // PacketSinkApplication on one host
    Address sinkAddress(InetSocketAddress(Ipv4Address("10.0.47.2"), port));
    PacketSinkHelper sink("ns3::TcpSocketFactory", sinkAddress);
    ApplicationContainer sinkApp = sink.Install(hosts.Get(hosts.GetN() - 1)); // Example: host 1
    sinkApp.Start(Seconds(1.0));
    sinkApp.Stop(Seconds(10.0));

    // BulkSendApplication on another host
    BulkSendHelper source("ns3::TcpSocketFactory", sinkAddress);
    source.SetAttribute("MaxBytes", UintegerValue(maxBytes)); // Unlimited data
    ApplicationContainer sourceApp = source.Install(hosts.Get(0)); // Example: host 0
...
uint32_t numFlows = 5; // Number of flows to simulate
...
for (uint32_t i = 0; i < numFlows; ++i) {
    ...
    // Install BulkSend on the sender
    BulkSendHelper sourceHelper("ns3::TcpSocketFactory", sinkAddress);
    sourceHelper.SetAttribute("MaxBytes", UintegerValue(0)); // Unlimited data
    sourceHelper.SetAttribute("SendSize", UintegerValue(1000)); // Packet size
...
for (uint32_t i = 0; i < sinkApps.size(); ++i) {
    ...
    Simulator::Schedule(Seconds(4.0), [sink, i]() {
        double throughput = sink->GetTotalRx() * 8.0 / (9.0 * 1e6); // Throughput in Mbps
...
//     pointToPointSR.SetQueue("ns3::DropTailQueue", "MaxSize", StringValue("60p")); // max queue size is 60 packet
...
//     pointToPointT.SetQueue("ns3::DropTailQueue", "MaxSize", StringValue("60p")); // max queue size is 60 packet
...
// ////////////////////////////////
//     rxS1R1Throughput.open("buffered-tcp-example-s1-r1-throughput.dat", std::ios::out);
//     rxS1R1Throughput << "#Time(s) flow thruput(Mb/s)" << std::endl;
//     rxS2R2Throughput.open("buffered-tcp-example-s2-r2-throughput.dat", std::ios::out);
//     rxS2R2Throughput << "#Time(s) flow thruput(Mb/s)" << std::endl;
//     rxS3R1Throughput.open("buffered-tcp-example-s3-r1-throughput.dat", std::ios::out);
//     rxS3R1Throughput << "#Time(s) flow thruput(Mb/s)" << std::endl;
//     fairnessIndex.open("buffered-tcp-example-fairness.dat", std::ios::out);
//     t1QueueLength.open("buffered-tcp-example-t1-length.dat", std::ios::out);
//     t1QueueLength << "#Time(s) qlen(pkts) qlen(us)" << std::endl;
//     t2QueueLength.open("buffered-tcp-example-t2-length.dat", std::ios::out);

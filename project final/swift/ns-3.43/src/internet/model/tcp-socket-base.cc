...
TcpSocketBase::EnterCwr(uint32_t currentDelivered)
{
    ...
    m_tcb->m_cWndInfl = m_tcb->m_ssThresh;   ///ask
...
TcpSocketBase::EnterRecovery(uint32_t currentDelivered)
{
    ...
    m_tcb->m_ssThresh = m_congestionControl->GetSsThresh(m_tcb, bytesInFlight);  ///ask
...
TcpSocketBase::ReceivedAck(Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
    ...
    if (m_tcb->m_congState == TcpSocketState::CA_CWR && (ackNumber > m_recover))
    {
        ...
        if (!m_congestionControl->HasCongControl())
        {
            m_tcb->m_cWnd = m_tcb->m_ssThresh.Get();  ///ask
                          ...
                          bool receivedData)
{
    ...
    else if (ackNumber > oldHeadSequence)
    {
        ...
        ///// may be relevante for recovery in swift
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////swift modification////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // for swift modifacation, as swift logic decreasing the window per RTT
  
    //swiftMod
    if(1)
    {
        uint32_t cwnd_prev = m_tcb->m_cWnd;
        bool can_decrease = m_tcb->CanDecrease();

        m_tcb->m_retransmit_count++;

        if (m_tcb->m_retransmit_count >= m_retxThresh)
        {
            m_tcb->m_cWnd = m_tcb->m_min_cwnd;
        }
        else
        {
            if (can_decrease)
            {
                m_tcb->m_cWnd = (1 - m_tcb->m_max_mdf)*m_tcb->m_cWnd;
            }
        }

        m_tcb->Clamp();
        if (m_tcb->m_cWnd <= cwnd_prev)
        {
            m_tcb->m_t_last_decrease = Simulator::Now();
        }
        
        m_tcb->m_cWndInfl = m_tcb->m_cWnd;


        if (m_tcb->m_cWnd < m_tcb->m_segmentSize)
        {

            double temp_rtt = m_tcb->m_lastRtt.Get().GetSeconds();
            if (temp_rtt < 0.001)
            {
                temp_rtt = 0.001;
            }
            
            m_tcb->m_pacingRate = (m_tcb->m_cWnd * 8)/ temp_rtt;

...
int main(int argc, char *argv[]) 
{
    ...
    if(protocol == "TcpSwift")
    {
        ...
        enable_swift = true;
        threshold_k = 2666;
        Config::SetDefault("ns3::TcpSwift::Mode", UintegerValue(swift_mode));
    }
    else // dctcp
    {
    ...
    //Config::SetDefault("ns3::TcpSocketState::EnableSwift",  BooleanValue(enable_swift));
    //Config::SetDefault("ns3::TcpSocketState::EnablePacing",  BooleanValue(enable_swift));
    Config::SetDefault("ns3::TcpSocketState::EnablePacing",  BooleanValue(enable_swift));
...
//--------------------------flow monitor---------------------------
    ...
    for (auto iter = stats.begin (); iter != stats.end (); ++iter) 
    {
        ...
        else
        {
            if (swift_mode == 0)
            {
                filename = "scratch/results_swift.txt";
            }
            else if (swift_mode == 1)
            {
                filename = "scratch/results_swift_epsilon.txt";
            }
            else{

                filename = "scratch/results_swift_0.025.txt";
            }
        }
